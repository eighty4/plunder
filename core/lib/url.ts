...
export async function getBaseHref(page: Page): Promise<string | null> {
    ...
    if (/^https?:\/\//.test(trimmed)) {
        ...
        throw new Error(
            'not sure how a <base href=""/> without a leading slash is used by browsers',
        )
    } else {
        return trimmed
    }
...
export function rewriteHref(
    ...
    if (href.startsWith('/')) {
        ...
        if (pageUrl.pathname === '/') {
            ...
            const pagePath = pageUrl.pathname.endsWith('/')
                ? pageUrl.pathname.substring(0, pageUrl.pathname.length - 1)
                : pageUrl.pathname
            const subpath = pagePath.substring(0, pagePath.lastIndexOf('/'))
            return createUrl(
                pageUrl.protocol,
                pageUrl.host,
                path.join(subpath, href),
            )
        }
    } else if (baseHref === '/') {
        return createUrl(pageUrl.protocol, pageUrl.host, href)
